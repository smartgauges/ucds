...
void dwc_poll(usbd_device *usbd_dev)
{
	...
	if (intsts & OTG_GINTSTS_EOPF) {
		if (usbd_dev->user_callback_eopf) {
			usbd_dev->user_callback_eopf();
		}
		REBASE(OTG_GINTSTS) = OTG_GINTSTS_EOPF;
	}
	...
	if (usbd_dev->user_callback_eopf) {
		REBASE(OTG_GINTMSK) |= OTG_GINTMSK_EOPFM;
	} else {
		REBASE(OTG_GINTMSK) &= ~OTG_GINTMSK_EOPFM;
	}
}
